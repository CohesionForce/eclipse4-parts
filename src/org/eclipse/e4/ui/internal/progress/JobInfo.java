...
import org.eclipse.e4.ui.internal.progress.SubTaskInfo;
...
public class JobInfo extends JobTreeElement {
	...
	private List<SubTaskInfo> children = Collections
			.synchronizedList(new ArrayList<SubTaskInfo>());
	 ...
	 */
	public int compareTo(JobInfo element) {
		...
		return (thisState > anotherState ? -1 : (thisState == anotherState ? 0
				: 1));
	}
	 ...
	 * 
	 * @see
	 * org.eclipse.ui.internal.progress.JobTreeElement#getCondensedDisplayString
	 * ()
	 */
	 ...
	 * 
	 * @see
	 * org.eclipse.ui.internal.progress.JobTreeElement#getDisplayString(boolean)
	 */
	String getDisplayString(boolean showProgress) {
		String name = getDisplayStringWithStatus(showProgress);
		if (job.isSystem()) {
			return NLS.bind(ProgressMessages.JobInfo_System,
					(new Object[] { name }));
		}
		return name;
	}
	...
	private String getDisplayStringWithStatus(boolean showProgress) {
		if (isCanceled()) {
			return NLS.bind(ProgressMessages.JobInfo_Cancelled,
					(new Object[] { getJob().getName() }));
		}
		if (isBlocked()) {
			return NLS.bind(ProgressMessages.JobInfo_Blocked, (new Object[] {
					getJob().getName(), blockedStatus.getMessage() }));
		}
		...
		if (getJob().getState() == Job.SLEEPING) {
			return NLS.bind(ProgressMessages.JobInfo_Sleeping,
					(new Object[] { getJob().getName() }));
		}

		return NLS.bind(ProgressMessages.JobInfo_Waiting,
				(new Object[] { getJob().getName() }));
