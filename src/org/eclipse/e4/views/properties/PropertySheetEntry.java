...
public class PropertySheetEntry extends EventManager implements
	...
	private List<IPropertyDescriptor> computeMergedPropertyDescriptors() {
		...
		ArrayList<Map<Object, IPropertyDescriptor>> propertyDescriptorMapList = new 
				ArrayList<Map<Object, IPropertyDescriptor>>();
		for (int i = 0; i < values.length; i++) {
			...
			propertyDescriptorMapList.add(computePropertyDescriptorsFor(source));
		}

		// intersect
		Map<Object, IPropertyDescriptor> intersection = propertyDescriptorMapList.get(0);
		for (int i = 1; i < propertyDescriptorMapList.size(); i++) {
			// get the current ids
			Object[] ids = intersection.keySet().toArray();
			for (int j = 0; j < ids.length; j++) {
				Object object = propertyDescriptorMapList.get(i).get(ids[j]);
				if (object == null ||
	...
	protected IPropertySource getPropertySource(Object object) {
		...
		if (provider == null && object != null && object instanceof IAdaptable) {
			IAdaptable adaptable = (IAdaptable) object;
			provider = (IPropertySourceProvider) adaptable
		...
		if (provider != null) {
			...
			IAdaptable adaptable = (IAdaptable) object;
			result = (IPropertySource) adaptable
	...
	private void refreshChildEntries() {
		...
		Map<Object, PropertySheetEntry> entryCache = new HashMap<Object, PropertySheetEntry>(childEntries.length * 2 + 1);
		...
		List<PropertySheetEntry> entriesToDispose = new ArrayList<PropertySheetEntry>(Arrays.asList(childEntries));
